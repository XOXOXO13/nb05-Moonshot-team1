...
import { CreateCommentDto, TaskCommentDto, updateCommentDto } from "../../inbound/requests/comment-req-dto";
...
import { PersistComment, PersistCommentEntity } from "../entities/comment/comment-entity";
...
export class CommentService implements ICommentService {
  ...
  async createComment(dto: CreateCommentDto): Promise<CommentResDto> {
    ...
    const comment = await this._unitOfWork.repos.commentRepository.create(taskId, userId, content);
  ...
  async getCommentList(dto: TaskCommentDto): Promise<CommentResDto[]> {
    ...
    const comments = await this._unitOfWork.repos.commentRepository.findCommentsByTask(taskId, page, limit);
  ...
  async getCommentDetail(commentId: number): Promise<CommentResDto> {
    const comment = await this._unitOfWork.repos.commentRepository.findById(commentId);
    if (!comment) {
  ...
  async editComment(dto: updateCommentDto): Promise<CommentResDto> {
    ...
    const comment = await this._unitOfWork.repos.commentRepository.update(commentId, content);
    return CommentMapper.toResDto(comment);
  }
