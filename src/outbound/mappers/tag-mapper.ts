...
export class TagMapper {
    ...
    static toCreateData(entities: NewTagEntity[]) {
        const createTagData: CreateTagData[] = entities.map((entity: NewTagEntity) => {
    ...
    static toCreateEntities(tags: string[]) {
        return tags.map((tagName: string) => { return TagEntity.createNew(tagName) });
    }

    static toPersistEntites(records: {
        id: number;
        name: string;
    }[]): PersistTagEntity[] {
